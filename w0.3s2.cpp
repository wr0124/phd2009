...
int main()
{
  ...
  for(int i=0; i<N-1;i++)
    {
      Net.insert(pairnode::value_type( i,i+1 ) );
    }
  ...
  for( maptr=Net.begin(); maptr!=Net.end(); maptr++ )
    {
      ...
      ERN.insert( pairnode::value_type(maptr->first,maptr->second));
    }
  ...
  for(int  fBoundCof=1; fBoundCof<11; fBoundCof+=1)
    {
      Nfile=fBoundCof;
      ...
      for(int i=0;i<Sample;i++)
	{
	  Net.clear();
	  NewNet.clear();
	  for( maptr=ERN.begin();maptr!=ERN.end();maptr++)
	    {
	  ...
	  while(NoChange<1 && update<Tmax)
	      ...
	      for(int i=0;i<NodeChoseSquare.size();i++)
		{
		  ...
		  if(ran>wire)
		      ...
		      if( fabs(nodeS-nodePS)<=BoundConf)
			{
			  ...
			  for( vtr=NodeChoseNeib.begin(); vtr!=NodeChoseNeib.end(); vtr++)
			    {
			      State0[*vtr]=average;			 
			    }
			  for( vtr=NodeChosePNeib.begin(); vtr!=NodeChosePNeib.end(); vtr++)
			    {
			      State0[*vtr]=average;			   
			    }
			  for(maptr=Net.begin();maptr!=Net.end();maptr++)
		  ...
		  else 
		    {
		      if(  fabs( nodeS-nodePS) >  BoundConf)
			{
			   cout<<"fBoundCof is  for rewire "<<fBoundCof<<endl;		  
	      ...
	      if( PersonState!=State[Person])
		{
	      ...
	      for(maptr=Net.begin();maptr!=Net.end();maptr++)
		{
		  if(   fabs(State0[maptr->first]-State0[maptr->second])> FinialS  )
		    {
		      Dis++;
		    }
		}
